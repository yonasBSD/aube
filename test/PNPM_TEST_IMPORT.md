...
- [ ] `pnpm/test/install/misc.ts` (37 tests, 645 LOC) → [test/pnpm_install_misc.bats](pnpm_install_misc.bats) (10/37 ported)
  ...
  - Remaining high-value: strict-store-pkg-content-check (516 — pnpm tampers with the cached manifest in the store and expects the next install to fail; aube parity depends on whether aube has an equivalent integrity check), CI-mode env override, install timeout, top-level-plugins (190 — needs `standard` package, not in registry).
  ...
  - Documented divergences (don't port without aube-side fix): `package.yaml` manifest (136 — aube's manifest reader is `package.json`-only; would need a multi-crate format-aware rewrite), `--lockfile-dir` (112 — aube has no flag for placing the lockfile outside the project root).
